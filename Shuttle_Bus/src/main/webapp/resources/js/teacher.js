$(document).ready(function() {
		...
		var user_id="t25qhs93e8aj3jh";
		var see_more_notification=0;
		var mySchedule_data; // use for my schedule form
		var permission_request_data; //user for permission form
		...
		teacher_notification();
   		function teacher_notification(){
			...
			var emergency=emergency_request();
			...
			for(i=notification.length-1;i>notification.length-8&&i>=0;i--){
				...
				case 'permission_waiting':
					notification_form+='<li><a class="detial_permission" value="'+notification[i].permission_id+'" href="#confirm_permission">You get a request for  '+notification[i].student_fullname+' !<span class="'+notification[i].notification +' badge right"></span></a></li><li class="divider"></li>';
					break;	
			...
			$('.detial_permission').click(function(){
				var permission_id=$(this).attr('value');
				console.log(permission_id);
				confirm_permission(permission_id);
			})
   		}
   		...
   		function see_more(){
			...
			//.log("Book "+permission[0].notification);
   			for(i=0;i<exchange_seat_nitif.length;i++){
				notification.push(exchange_seat_nitif[i]);
			}
			for(i=0;i<permission.length;i++){
			...
			for(i=notification.length-1;i>notification.length-((see_more_notification*10)+1)&&i>=0;i--){
				...
				case 'exchange_seat':
					...
					break;
				case 'permission_waiting':
					notification_form+='<li class="collection-item"><p><a value="'+notification[i].permission_id+'" class="detial_permission" href="#confirm_permission">You get a request from '+notification[i].student_fullname+' Batch '+notification[i].batch_number+' at '+notification[i].date_of_request+' by this reason "'+notification[i].reason+'"!<span class="'+notification[i].notification +' badge right"></span></a><p></li><li class="divider"></li>';
					break;
				case 'emergency_approved':
   		...
   		function permission_noti(){
			var submit={"user_id":user_id,"value":see_more_notification};
   			$.ajax({
   				...
   				cache:false,
   				type : "POST",
   				url : "permission_teacher_noti",
   				contentType : "application/json",
   				data : JSON.stringify(submit),
   				timeout : 100000,
   				success : function(data) {
   						permission_request_data=data;
   				},
   				error : function(e) {
   					console.log("ERROR: ", e);
   			...
   			return permission_request_data;
   		}
		...
		function mySchedule(){
   		...
   		//set confirm permission form
		function confirm_permission(permission_id){
			var index=null;// to find the index of permission_request_data
			for(j=0;j<permission_request_data.length;j++){
				if(permission_request_data[j].permission_id==permission_id){
					index=j;
				}
			}
			console.log("Index: "+index);
			var confirm_form='<div id="get_permission_confrim" class="modal-content"><h5 class="center sch light-blue-text">Confirm Student Request</h5><br><table  class="centered bordered highlight">'
								+'<tr><td> Student\'s Name: </td><td>'+ permission_request_data[index].student_fullname
								+'</td><tr><td> Student\'s Username: </td><td>'+ permission_request_data[index].student_username
								+'</td><tr><td> Batch: </td><td>'+ permission_request_data[index].batch_number
								+'</td><tr><td> Date of request: </td><td>'+ permission_request_data[index].date_of_request
								+'</td><tr><td> Reason: </td><td>'+ permission_request_data[index].reason
								+'</td></table></div>'
								+'<div class="modal-footer">'
		   						+'<a value="'+index+'" class="modal-action modal-close waves-effect waves-green btn-flat permission_approve">Approve</a>'
		    					+'<a value="'+index+'" class="modal-action modal-close waves-effect waves-green btn-flat permission_ignore">Ignore</a></div>'
			document.getElementById('confirm_permission').innerHTML=confirm_form;
			$('.permission_approve').click(function(){
			 	var index=$(this).attr('value');
			 	var status="permission_approved"
			 	teacher_confirm_permission(permission_request_data[index].student_id, permission_request_data[index].date_of_request,status);
			 })
			$('.permission_ignore').click(function(){
			 	var index=$(this).attr('value');
			 	var status="permission_ignored"
			 	teacher_confirm_permission(permission_request_data[index].student_id, permission_request_data[index].date_of_request,status);
			 })
			
		}
		//confirm_permission();
		function teacher_confirm_permission(student_id, date_of_request,status){
			var submit={"user_id":student_id,"date_of_request":date_of_request,"status":status};
			console.log(submit);
   			$.ajax({
   				async:false,
 				cache:false,
   				type : "POST",
   				url : "teacher_confirm_permission",
   				contentType : "application/json",
   				data : JSON.stringify(submit),
   				timeout : 100000,
   				success : function(data) {
...
//To check whether can exchange or not
		...
		$.ajax({
			...
			success : function(data) {
					...
					if(data==null||data==""){
						...
						var exch='<li><a id="req_exchange" href="#exchangeSeat">Exchange Seat</a></li><li class="divider"></li>';
	...
	//schedule for emergency
	 ...
	 //When Student Permission click
	$('#list_permission').click(function(){
		var list_stu_data=[];
		console.log("Permission Student");
	  	$.ajax({
			type : "GET",
		 	url : "list_student_permission",
		 	contentType : "application/json",
			timeout : 100000,
		 	success : function(data) {
		 			list_stu_data=data;
					console.log(list_stu_data);
					var list_stu_form="";
	   				for(i=0;i<data.length;i++){	
		   				list_stu_form+='<tr><td>'+(i+1)
		   								+'</td><td>'+data[i].full_name
		   								+'</td><td>'+data[i].username
		   								+'</td><td>Batch '+data[i].batch_number
		   								+'</td><td>'+data[i].date_of_request
		   								+'</td><td><span class="userDetail" value="'+i
		   						        +'" style="color:#ff9800;cursor:pointer">reset</span></td></tr>';	
	   					}
	   					document.getElementById('get_list_student').innerHTML=list_stu_form;
			},
		 	error : function(e) {
		 		console.log("ERROR: ", e);
